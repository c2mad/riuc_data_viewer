...
export default function ArticulosCientificos() {
  ...
  // Mapeo de campos a sus labels para mostrar en el placeholder
  const campoLabels = {
    titulo: "título",
    autor: "autor", 
    anio: "año"
  ...
  // ----------------- búsqueda -----------------
  // Maneja la búsqueda y el campo seleccionado para filtrar los artículos
  const handleSearch = (texto, campoBusqueda) => {
    // Actualiza el término de búsqueda
    setQuery(texto);

    // Actualiza el campo de búsqueda (por defecto mantiene el actual)
    setCampo(campoBusqueda || campo);

    // Solo reinicia la paginación si el texto o el campo realmente han cambiado
    // Esto evita que se reseteé innecesariamente a la página 1 al hacer clic en la paginación
    if (texto !== query || campoBusqueda !== campo) {
      setCurrentPage(1);
    }
  };
  ...
  // ----------------- render -----------------
  return (
    <main className="relative min-h-screen bg-gray-100 px-0 sm:px-4 py-6 sm:py-10">
      {/* Header */}
      <div className="text-center mt-4 sm:mt-8 mb-4 sm:mb-6 px-2 sm:px-0">
        <h5 className="text-sm sm:text-base md:text-lg text-red-500 mb-1 font-semibold">
          RIOUC
        </h5>
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-semibold p-2 sm:p-5 bg-gradient-to-r from-black to-black bg-clip-text text-transparent border-b-2 border-red-400 inline-block">
          ARTÍCULOS CIENTÍFICOS
        </h1>
        <p className="text-xs sm:text-sm text-gray-400 mt-4 max-w-sm sm:max-w-xl mx-auto px-2 text-center">
            Explora los últimos artículos y publicaciones científicas de los distintos observatorios que conforman la RIOUC.
          </p>
      </div>
      {/* Barra de búsqueda y filtro burger */}
      <div className="mx-auto mb-6 sm:mb-8 w-full max-w-full sm:max-w-4xl px-2 sm:px-4 flex flex-row items-center justify-center gap-2 sm:gap-4">
        <div className="flex-1 max-w-md">
          <Search
            onSearch={handleSearch}
            campo={campo}
            placeholder={`Buscar por ${campoLabels[campo]}...`}
          />
        </div>
      ...
      {/* Lista */}
      <div className="bg-white p-2 sm:p-10 rounded-none sm:rounded-xl shadow-none sm:shadow-xl w-full sm:max-w-7xl mx-auto">
