...
import { lista_videos } from "../../utils/modelo_videos";
...
export default function videos() {
  ...
  const totalPages = Math.ceil(lista_videos.length / projectsPerPage);
  ...
  const handleSearch = (query) => {
    const filteredProjects = lista_videos.filter((project) =>
  ...
  const projectsToDisplay =
    searchResults.length > 0
      ...
      : lista_videos.slice(startIndex, endIndex);
